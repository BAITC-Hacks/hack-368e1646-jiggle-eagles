...
Saved analyses retain their original calculations and exports. When a saved run predates temporal evidence or observed patterns, the account card marks that evidence as unavailable; analyze the input files again to calculate it.
...
- **Deferred capabilities:** dedicated cluster browsing, advanced filters, transaction viewing, cycle analysis, node-removal simulation, AI chat, and full-network force-layout visualization are not implemented. Day-level FIFO allocation measures compatible timing, not fund identity; same-day ordering remains unknown.
