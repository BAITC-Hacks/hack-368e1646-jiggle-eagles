...
The multi-stage image uses the same pinned Node version and runs as the non-root `node` user. Its build context allows only exact app build inputs, excludes `.env` and key files, and passes no configuration through build arguments. Add each new source file or asset explicitly to `.dockerignore` when needed by the build. The OpenAI helper is included; the optional TypeScript connectivity CLI is a native development command and is not shipped in the runtime image. Keep credentials out of source code; the connectivity app needs none.
