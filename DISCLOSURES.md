...
- Raw inputs, downloaded sources, generated account-level exports and test screenshots are stored only in ignored `data/private/money-graph/` in this workspace. The dataset is authorized for hackathon use, not implicitly for redistribution. Unrelated private files were preserved. No external customer enrichment was performed, and no competition data was submitted to the application's optional legacy OpenAI integration.
...
Exact Python runtime and test pins are in `requirements-money-graph.txt` and `requirements-money-graph-test.txt`. pandas and PyArrow handle Parquet; NetworkX handles graph operations; Python's standard library serves the dashboard. There are no external fonts, CDN scripts or UI analytics. Playwright/Chromium are test-only.
...
Remote GitHub Actions for these changes, a second-machine clean setup, Docker execution and a live organizer presentation remain unverified. The existing CI covers the Node starter; Python/browser commands are documented for local execution. No deployment or push was performed by Codex for this implementation.
