...
| Docs/domain agent | Assigned files in `docs/energy/**` and `docs/hackathon/**`. |
...
- Map confirmed requirements to implementation and verification in [requirements](docs/hackathon/requirements.md). Do not invent task requirements or dataset semantics. Before domain calculations, read [docs/energy/README.md](docs/energy/README.md), the relevant linked calculation/validation references, and actual dataset/provider documentation. These references do not establish official requirements or dataset semantics. Record units, measurement type, interval/timezone, scope and provenance; block dependent calculations when semantics remain unresolved.
