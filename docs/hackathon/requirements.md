...
| ENG-03 | L: pinned, isolated local setup; clear launch path; Python CI and accurate setup/data documentation. | A fresh environment uses `.python-version` and exact package pins; wrong runtimes, dependency drift and shared/global environments fail before launch; compatible environments are preserved; launch works from paths with spaces. CI uses the same setup and synthetic Python/browser tests. | `.python-version`, `scripts/setup.sh`, `scripts/check_environment.py`, `scripts/money-graph.sh`, `tests/test_setup.py`, workflow and setup docs; 9 setup regressions and a fresh local dependency installation pass. Remote Actions and a second-machine rehearsal remain unverified. |
